...
namespace ManagedDoom
{
	public sealed class DoomGame
	{
		...
		{
			options.Skill = (GameSkill)DoomMath.Clamp((int)skill, (int)GameSkill.Baby, (int)GameSkill.Nightmare);

			if (options.GameMode == GameMode.Retail)
			{
				options.Episode = DoomMath.Clamp(episode, 1, 4);
			}
			...
			if (options.GameMode == GameMode.Commercial)
			{
				options.Map = DoomMath.Clamp(map, 1, 32);
			}
			else
			{
				options.Map = DoomMath.Clamp(map, 1, 9);
			}
