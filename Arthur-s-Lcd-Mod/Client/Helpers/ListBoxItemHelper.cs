...
namespace LcdMod.Client.Helpers
{
    public static class ListBoxItemHelper
    {
        ...
        public static long GetLongUserData(MyTerminalControlListBoxItem item)
        {
            if (item == null || item.UserData == null)
                return 0L;

            if (item.UserData is long)
            ...
            long value;
            return long.TryParse(item.UserData.ToString(), out value) ? value : 0L;
        }
