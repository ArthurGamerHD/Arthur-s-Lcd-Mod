    ...
    internal sealed class PercentageApp<TEntry>
    {
        ...
        void DrawList(List<MySprite> sprites, List<TEntry> entries)
        {
            ...
            for (int i = 0; i < showCount; i++)
                DrawRow(sprites, entries[start + i], shouldScroll);
        }
        ...
        void DrawGridCell(List<MySprite> frame, TEntry entry, float xStart, float xEnd, float yStart, float rowHeight)
        {
            ...
            frame.Add(new MySprite { Type = SpriteType.TEXT, Data = name.ToString(), Position = new Vector2(nameRect.X + 2f * _owner.Scale, nameRect.Y + 2f * _owner.Scale), RotationOrScale = .9f * _owner.Scale * _owner.FontScaleInternal, Color = _owner.SurfaceInternal.ScriptForegroundColor, Alignment = TextAlignment.LEFT, FontId = "White" });
            ...
            var barInnerPaddingX = 2f * _owner.Scale;
            ...
            BarPanel.CreateSprites(frame, new Vector2(barRect.X + barInnerPaddingX, barRect.Y + barInnerPaddingY + (2f * _owner.Scale)), new Vector2(Math.Max(1f, barRect.Width - 2f * barInnerPaddingX), Math.Max(1f, barRect.Height - 2f * barInnerPaddingY)), fillColor, fillColor.DeriveAccentColor(.6f, 0.7), pct, _owner.GetEntryUsageColorInternal(pct));
            frame.Add(new MySprite { Type = SpriteType.TEXT, Data = _owner.GetNumberInternal(pct), Position = new Vector2(textRect.Right - (2f * _owner.Scale), textRect.Y + 2f * _owner.Scale), RotationOrScale = .95f * _owner.Scale * _owner.FontScaleInternal, Color = _owner.SurfaceInternal.ScriptForegroundColor, Alignment = TextAlignment.RIGHT, FontId = "White" });
        }
        ...
        void DrawScrollBar(List<MySprite> frame, float scale, float initialY, float viewportHeight, float scrollBarCenter, float scrollBarHeight)
        {
            ...
            DrawCapsule(frame, trackCenter, barWidth, viewportHeight,
                new Color(_owner.SurfaceInternal.ScriptForegroundColor.R, _owner.SurfaceInternal.ScriptForegroundColor.G, _owner.SurfaceInternal.ScriptForegroundColor.B, 127));
            ...
            DrawCapsule(frame, thumbCenter, barWidth, scrollBarHeight,
                new Color(_owner.HeaderColorInternal.R, _owner.HeaderColorInternal.G, _owner.HeaderColorInternal.B, 250));
        }
