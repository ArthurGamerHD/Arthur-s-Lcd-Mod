...
namespace LcdMod.Client.SurfaceScripts.Abstract
{
    internal sealed class PercentageApp<TEntry>
    {
        ...
        {
            var rowHeight = LINE_HEIGHT * _owner.ConfiguredScale;
        ...
        {
            var rowHeight = 2f * LINE_HEIGHT * _owner.ConfiguredScale;
            var viewportAvailableHeight = _owner.ViewBox.Height - (_owner.CaretYInternal - _owner.ViewBox.Y) - _owner.FooterHeightInternal;
            int maxRows = Math.Max(1, (int)Math.Floor(viewportAvailableHeight / rowHeight));
        ...
        void DrawRow(List<MySprite> frame, TEntry entry)
        {
            ...
            var clip = new Rectangle((int)position.X, (int)position.Y, (int)(_owner.ViewBox.Width - position.X + _owner.ViewBox.X - 145 * _owner.ConfiguredScale), (int)(LINE_HEIGHT * _owner.ConfiguredScale));
            var barMargin = 8 * _owner.ConfiguredScale;
            Vector2 size = new Vector2(_owner.ViewBox.Width - position.X + _owner.ViewBox.X, clip.Height) - barMargin;

            BarPanel.CreateSprites(frame, new Vector2(clip.Location.X, clip.Location.Y + _owner.ConfiguredScale) + barMargin / 2f, size, _owner.GetEntryBarFillColorInternal(), _owner.GetEntryBarBackgroundColorInternal(), pct, _owner.GetEntryUsageColorInternal(pct));
            frame.Add(MySprite.CreateClipRect(clip));
            position.X += 16 * _owner.ConfiguredScale;
            position.Y += 4 * _owner.ConfiguredScale;
            frame.Add(new MySprite { Type = SpriteType.TEXT, Data = _owner.GetEntryNameInternal(entry), Position = position, RotationOrScale = _owner.ConfiguredScale * _owner.FontScaleInternal, Color = _owner.SurfaceInternal.ScriptForegroundColor, Alignment = TextAlignment.LEFT, FontId = "White" });
            frame.Add(MySprite.CreateClearClipRect());
            position.X = _owner.ViewBox.Width + _owner.ViewBox.X;
            frame.Add(new MySprite { Type = SpriteType.TEXT, Data = _owner.GetNumberInternal(pct), Position = position, RotationOrScale = _owner.ConfiguredScale * _owner.FontScaleInternal, Color = _owner.SurfaceInternal.ScriptForegroundColor, Alignment = TextAlignment.RIGHT, FontId = "White" });
            _owner.CaretYInternal += LINE_HEIGHT * _owner.ConfiguredScale;
        }

        void DrawGridCell(List<MySprite> frame, TEntry entry, float xStart, float xEnd, float yStart, float rowHeight)
        {
            var cellPadding = (LINE_HEIGHT * _owner.ConfiguredScale) / 3f;
            var pct = MathHelper.Clamp(_owner.GetEntryPercentageInternal(entry), 0f, 1f);
            ...
            if (!_owner.DrawLinesInternal)
            {
                ...
                BorderRenderer.CreateSpritesFromRect(dropShadow, frame, hsv.HSVtoColor(),
                    radiusScale: _owner.ConfiguredScale);
            ...
            frame.Add(new MySprite { Type = SpriteType.TEXT, Data = name.ToString(), Position = new Vector2(nameRect.X + 2f * _owner.ConfiguredScale, nameRect.Y + 2f * _owner.ConfiguredScale), RotationOrScale = .9f * _owner.ConfiguredScale * _owner.FontScaleInternal, Color = _owner.SurfaceInternal.ScriptForegroundColor, Alignment = TextAlignment.LEFT, FontId = "White" });
            ...
            var barInnerPaddingX = 2f * _owner.ConfiguredScale;
            ...
            BarPanel.CreateSprites(frame, new Vector2(barRect.X + barInnerPaddingX, barRect.Y + barInnerPaddingY + (2f * _owner.ConfiguredScale)), new Vector2(Math.Max(1f, barRect.Width - 2f * barInnerPaddingX), Math.Max(1f, barRect.Height - 2f * barInnerPaddingY)), fillColor, fillColor.DeriveAccentColor(.6f, 0.7), pct, _owner.GetEntryUsageColorInternal(pct));
            frame.Add(new MySprite { Type = SpriteType.TEXT, Data = _owner.GetNumberInternal(pct), Position = new Vector2(textRect.Right - (2f * _owner.ConfiguredScale), textRect.Y + 2f * _owner.ConfiguredScale), RotationOrScale = .95f * _owner.ConfiguredScale * _owner.FontScaleInternal, Color = _owner.SurfaceInternal.ScriptForegroundColor, Alignment = TextAlignment.RIGHT, FontId = "White" });
        }
