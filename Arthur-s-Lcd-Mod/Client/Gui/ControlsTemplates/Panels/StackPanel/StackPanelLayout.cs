...
{
    public sealed class StackPanelLayout
    {
        StackPanelLayout()
        {
        }
        ...
        public static StackPanelLayout Create(
            RectangleF contentBounds,
            float rowHeight,
            int itemCount,
            int startIndex = 0)
        {
            var panel = new StackPanelLayout();
        ...
        public static float CalculateTotalHeight(float rowHeight, float gap, int itemCount)
        {
            int count = Math.Max(0, itemCount);
            if (count == 0)
                return 0f;

            return count * Math.Max(1f, rowHeight) + Math.Max(0, count - 1) * Math.Max(0f, gap);
        }
    }
}
