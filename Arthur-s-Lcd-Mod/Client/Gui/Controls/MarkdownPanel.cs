    ...
    public static class MarkdownPanel
    {
        ...
        sealed class Renderer
        {
            ...
            void RenderCodeBlock(CodeBlock block, float indent)
            {
                ...
                string text = NormalizeNewlines(block.Text ?? string.Empty);
            ...
            void CollectBlockRuns(BlockNode block, TextStyle style, List<TextRun> result)
            {
                ...
                if (code != null)
                {
                    AddTextRun(result, new TextRun(NormalizeNewlines(code.Text ?? string.Empty).Trim(), style.WithFont(CODE_FONT)));
                    return;
                }
            }
            ...
            {
                string text = NormalizeNewlines(run.Text);
                int index = 0;
                ...
                while (index < text.Length)
                {
                    ...
                    string token = _line.HasText ? " " + word : word;
                    AppendToken(new TextRun(token, run.Style), true);
                }
            }
            ...
            void AppendPreformattedRun(TextRun run)
            {
                ...
                for (int i = 0; i < text.Length; i++)
                {
                    ...
                    AppendToken(new TextRun(text[i].ToString(), run.Style), false);
                }
            }
            ...
            {
                return Math.Max(1f, Measure("Ag", fontId, scale).Y);
            }
